...
module Make( B : Buffer ) : sig
  ...
  let pop m i f =
    match M.find_first_opt (fun size -> size >= i) m with
    | Some (_i,[]) -> raise @@ Invalid_argument "Empty list in map"
    | Some (i,[b]) -> 
      M.remove i m, b
    | Some (i,b::bs) -> 
      let m' = M.remove i m |> M.add i bs in
  ...
  let alloc t size = 
    ...
    t.available <- avail';
    (* NOTE this will cause eventual buffer shrinkage, eventually resulting 
       in most buffers being incorrectly sized for the workload *)
    B.sub b 0 size
